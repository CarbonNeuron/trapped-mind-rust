//! Pet face mood states and ASCII art animation frames.
//!
...
//!
//! Eye characters use ASCII (`o`, `O`, `@`, `.`) to avoid alignment issues
//! with ambiguous-width Unicode symbols in different terminal emulators.
//! All frame lines are exactly 20 display cells wide.
...
// -- Animation frames: each mood has 2-4 frames of ASCII art --
// All lines are exactly 20 characters wide.
// Inner area between │...│ is 14 chars.
// Eye sockets: "  ┌──┐  ┌──┐  " (14 inner)
...
mod tests {
    ...
    #[test]
    fn test_frame_line_widths_consistent() {
        let all_frames: Vec<(&str, &[&[&str]])> = vec![
            ("idle", &IDLE_FRAMES),
            ("thinking", &THINKING_FRAMES),
            ("listening", &LISTENING_FRAMES),
            ("hot", &HOT_FRAMES),
            ("high_cpu", &HIGH_CPU_FRAMES),
            ("low_battery", &LOW_BATTERY_FRAMES),
            ("charging", &CHARGING_FRAMES),
        ];
        for (name, frames) in all_frames {
            for (fi, frame) in frames.iter().enumerate() {
                let expected = frame[0].chars().count();
                for (li, line) in frame.iter().enumerate() {
                    assert_eq!(
                        line.chars().count(), expected,
                        "{} frame {} line {} has {} chars, expected {}: {:?}",
                        name, fi, li, line.chars().count(), expected, line
                    );
                }
            }
        }
